...
from helpers.funtions import (read_template, load_dataset, data_preprocessing, make_segmentation, get_segments, print_results, load_lottiefile)

col1, col2 = st.columns([2, 5])
with col1:
    logo = load_lottiefile("1739232745617.json")
...
st.markdown(read_template("intro.md"))
...
with st.expander("Предобработка, фильтрация и объединение таблиц"):
    st.markdown(read_template("data_preproc.md") % (n_days, n_days))
...
st.markdown("Используя слайдеры, подберем границы групп по каждому из RFM признаков, контролируя по графикам распределение значений по двум другим признакам в каждой из групп")
# R
make_segmentation(df, cut_col='days_since_last_order', x='orders_count', y='order_sum', segments=segments, log_y=True, text='Число дней с последней покупки', key='R')
# F
make_segmentation(df, cut_col='orders_count', x='days_since_last_order', y='order_sum', segments=segments, log_y=True, text='Число покупок', key='F')
# M
make_segmentation(df, cut_col='order_sum', x='orders_count', y='days_since_last_order', segments=segments, log_y=False, text='Сумма покупок', key='M', exp_bins=True)
...
with st.expander("Анализ полученных результатов и предлагаемые способы взаимодействия с клиентами"):
    st.markdown(read_template("conclusion.md"))
